package ulan.menuterracotta.recyclerviewfastscroll;
...
import ulan.menuterracotta.recyclerviewfastscroll.viewprovider.DefaultScrollerViewProvider;
import ulan.menuterracotta.recyclerviewfastscroll.viewprovider.ScrollerViewProvider;
...
public class FastScroller extends LinearLayout {
            ...
            public boolean onTouch(View v, MotionEvent event) {
               // requestDisallowInterceptTouchEvent(true);
                if (event.getAction() == MotionEvent.ACTION_DOWN || event.getAction() == MotionEvent.ACTION_MOVE) {
                    ...
                    Log.e("KKKK",event.getX()+" "+event.getRawX());
                    v.setX((float) (event.getRawX()- Shared.margin_handle));
                  //  setScrollerPosition(relativePos);
                    setRecyclerViewPosition((float) (event.getRawX()-Shared.margin_handle));
                    return true;
    ...
    private void setRecyclerViewPosition(float relativePos) {
        ...
        recyclerView.getLayoutManager().scrollToPosition((int) relativePos);
        view.getWidth();
        counter = (view.getWidth()*recyclerView.getAdapter().getItemCount())/this.getWidth();
        if (recyclerView == null) return;
        int itemCount = recyclerView.getAdapter().getItemCount();
        Log.e("SSS",view.getWidth()*recyclerView.getAdapter().getItemCount()+" "+counter+" "+relativePos+" "+recyclerView.getY());
        int targetPos = (int) Utils.getValueInRange(0, itemCount - 1, (int) (relativePos * (float) itemCount));
        LinearLayoutManager linearLayoutManager = (LinearLayoutManager) recyclerView.getLayoutManager();
        linearLayoutManager.scrollToPositionWithOffset(0,-(int)(relativePos*counter));
      //  recyclerView.scrollBy((int)(relativePos*counter),(int) recyclerView.getY());
